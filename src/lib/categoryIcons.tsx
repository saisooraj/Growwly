...
// ── Emoji → icon name migration map (covers old EMOJI_PALETTE) ───────────────

export const EMOJI_TO_ICON_NAME: Record<string, string> = {
  // Food & Drink
  '🍕': 'IconPizza',      '🍔': 'IconMeat',          '🌮': 'IconToolsKitchen2',
  '🍜': 'IconToolsKitchen2', '🍣': 'IconFish',        '🥗': 'IconSalad',
  '🥪': 'IconBread',      '🥘': 'IconToolsKitchen2', '🍱': 'IconPackage',
  '🍰': 'IconCake',       '🧁': 'IconCake',           '🍩': 'IconCake',
  '☕': 'IconCoffee',      '🥤': 'IconCup',            '🧃': 'IconBottle',
  '🍺': 'IconBeer',       '🍷': 'IconGlass',
  // Transport
  '🚗': 'IconCar',        '🚕': 'IconCar',            '🚌': 'IconBus',
  '🚂': 'IconTrain',      '✈️': 'IconPlane',           '🛵': 'IconMotorbike',
  '🚲': 'IconBike',       '🛺': 'IconCar',             '⛽': 'IconGasStation',
  '🚢': 'IconShip',       '🚁': 'IconPlane',
  // Shopping & Style
  '🛍️': 'IconShoppingBag', '👗': 'IconShoppingBag',  '👟': 'IconShoppingBag',
  '👜': 'IconShoppingBag', '💄': 'IconMassage',       '⌚': 'IconDeviceWatch',
  '📦': 'IconPackage',    '🏷️': 'IconTag',             '🎁': 'IconGift',
  // Entertainment
  '🎬': 'IconMovie',      '🎮': 'IconDeviceGamepad2', '🎵': 'IconMusic',
  '🎸': 'IconMicrophone', '🎭': 'IconMoodSmile',      '🎲': 'IconDice',
  '⚽': 'IconBallFootball','🏀': 'IconBallFootball',   '🎯': 'IconTarget',
  '🏋️': 'IconBarbell',    '🧘': 'IconMassage',        '🏊': 'IconSwimming',
  // Home & Life
  '🏠': 'IconHome',       '🏢': 'IconBuilding',       '🛁': 'IconBath',
  '🔧': 'IconTool',       '💡': 'IconBulb',            '🛋️': 'IconSofa',
  '🌱': 'IconLeaf',       '🐶': 'IconDog',             '🐱': 'IconCat',
  // Health
  '💊': 'IconPill',       '🏥': 'IconBuildingHospital','🩺': 'IconStethoscope',
  '💉': 'IconVaccine',    '🧬': 'IconDna',             '❤️': 'IconHeart',
  '🦷': 'IconDental',     '👓': 'IconEyeglass',
  // Finance & Work
  '💰': 'IconCoin',       '💳': 'IconCreditCard',     '📈': 'IconTrendingUp',
  '💎': 'IconDiamond',    '🥇': 'IconMedal',           '🏦': 'IconBuildingBank',
  '💼': 'IconBriefcase',  '📊': 'IconChartBar',        '💻': 'IconDeviceLaptop',
  '📱': 'IconDeviceMobile','📚': 'IconBook',           '🎓': 'IconSchool',
  '🖥️': 'IconDeviceLaptop',
  // People & Social
  '🎊': 'IconConfetti',   '🤝': 'IconHeartHandshake', '👶': 'IconUsers',
  '💍': 'IconDiamond',    '🏖️': 'IconBeach',           '🌍': 'IconPlane',
  // Nature & Misc
  '🌿': 'IconLeaf',       '🌊': 'IconDroplet',        '⚡': 'IconBolt',
  '🌞': 'IconSun',        '🔥': 'IconFlame',           '❄️': 'IconSnowflake',
  '🌈': 'IconWaveSine',   '⭐': 'IconStar',            '🔑': 'IconKey',
  '📷': 'IconCamera',     '🎀': 'IconRosette',         '🧧': 'IconGift',
  '🪴': 'IconPlant',      '🏆': 'IconTrophy',          '🎪': 'IconConfetti',
}

/**
 * Converts an old emoji-prefixed custom category to the new icon-name format.
 * Returns the original string unchanged if it's already migrated or not emoji-prefixed.
 */
export function migrateEmojiCategory(val: string): string {
  if (!val) return val
  const spaceIdx = val.indexOf(' ')
  if (spaceIdx <= 0) return val
  const prefix = val.slice(0, spaceIdx)
  const name   = val.slice(spaceIdx + 1)
  if ((prefix.codePointAt(0) ?? 0) <= 255) return val  // not emoji
  if (prefix.startsWith('Icon')) return val             // already migrated
  const iconName = EMOJI_TO_ICON_NAME[prefix]
  return iconName ? `${iconName} ${name}` : val
}
